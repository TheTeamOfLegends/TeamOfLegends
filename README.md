...
2. Перед первым запуском: `node init.js` — создаст локальный `.env` из `.env.sample` и папку `tmp/pgdata`.
3. **Секреты:** файл `.env` не коммитится. В репозитории только `.env.sample` с тестовыми локальными значениями. Пароли/токены задаются через env.
